...
import kr.cleancode.dashboard.manager.web.check.service.*;
...
public class CheckFileDatabaseWeb {
	...
	private final boolean checkFileDatabase() {
		try {
			...
			paramVo.setRequest(String.format("Hello Web Database, %s", DateFormatUtils.format(System.currentTimeMillis(), "yyyy-MM-dd hh:mm:ss.SSS")));
	...
	private final boolean checkTables() {
		...
		try {
			...
			if(!checkWebSqlService.sTableExists(nameTable)) {
				...
				logger.info(String.format("[%s] Table[%s] creation results are: %b", XrManagerProperties.moduleName, nameTable, checkWebSqlService.sTableExists(nameTable)));
			}
	...
	private final boolean checkDatasBasic() {
		try {
			...
			logger.info(String.format("[%s] Root Account results are: %b", XrManagerProperties.moduleName, dataBasicWebSqlService.sRootAccountExists()));
