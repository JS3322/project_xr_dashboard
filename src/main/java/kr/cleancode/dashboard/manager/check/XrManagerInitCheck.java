...
public class XrManagerInitCheck {
	...
	public void check() {
		...
		logger.info(String.format("[%s] Try to identify the essential elements required for the program.", XrManagerProperties.moduleName));
		checkFileDatabaseWeb.check();
		checkFileDatabaseGlobal.check();
		checkFileDatabaseModule.check();
		logger.info(String.format("[%s] All essential elements required for the program have been identified.", XrManagerProperties.moduleName));
	}
	
	private void getSystemProperties() {
		logger.info(String.format("[%s] Bring the environmental configuration of the system.", XrManagerProperties.moduleName));
		final Properties properties = System.getProperties();
		final Iterator<Object> iterator = properties.keySet().iterator();
		while (iterator.hasNext()) {
			final Object key = iterator.next();
			if(!"line.separator".equals(key)) {
				logger.info(String.format("[%s] Key: %s, Value: \\%s.", XrManagerProperties.moduleName, key, properties.get(key)));
			}
		}
	}
}
