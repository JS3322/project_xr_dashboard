...
public class CheckBrforePerform {
	...
	private void directoryCheck() {
		...
		final File directoryOldModule = new File(directoryRoot,
				XrManagerProperties.getValueForJava("name.folder.module.old"));
	...
	private void folderCheck(final File trgtFolder) {
		logger.info(String.format("[%s] Confirmation of creating a required directory[%s].",
				XrManagerProperties.moduleName, trgtFolder.getPath()));
		if (trgtFolder.exists()) {
			logger.info(String.format("[%s] There is a required directory[%s].", XrManagerProperties.moduleName,
					trgtFolder.getPath()));
			return;
		}

		final boolean result = trgtFolder.mkdirs();
		logger.info(String.format("[%s] The result of creating a required directory[%s] is: %b.",
				XrManagerProperties.moduleName, trgtFolder.getPath(), result));
		if (!result) {
			System.exit(1);
		}
	}
